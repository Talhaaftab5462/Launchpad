...
ipcMain.handle('launch-game', async (event, { gameId, executablePath, steamAppId, processName }) => {
  if (runningGames.has(gameId)) return { success: false, error: 'Already running' }

  // Steam games: launch via URI so Steam handles DRM, overlay, etc.
  // Then poll tasklist for the exe name to track playtime.
  if (steamAppId) {
    const startTime = Date.now()
    runningGames.set(gameId, { process: null, startTime, polling: true, processName })
    if (win && !win.isDestroyed()) win.webContents.send('game-started', { gameId, startTime })
    await shell.openExternal(`steam://rungameid/${steamAppId}`)

    if (processName) {
      const waitAndPoll = async () => {
        let pid = null
        for (let i = 0; i < 120; i++) {
          await new Promise(r => setTimeout(r, 500))
          if (!runningGames.has(gameId)) return
          pid = await getProcessPid(processName)
          if (pid) break
        }
        if (!pid) { console.warn(`[launch] Steam game "${processName}" not found after 60s`); return }
        while (runningGames.has(gameId)) {
          await new Promise(r => setTimeout(r, 3000))
          if (!await getProcessPid(processName)) {
            const entry = runningGames.get(gameId)
            if (!entry) return
            const mins = Math.max(0, Math.round((Date.now() - entry.startTime) / 60000))
            runningGames.delete(gameId)
            if (win && !win.isDestroyed()) win.webContents.send('game-exited', { gameId, durationMins: mins, startTime: entry.startTime })
            return
          }
        }
      }
  ...
  try {
    ...
    if (isBatch) {
      // Launch the bat normally — we don't track cmd.exe itself
      ...
      runningGames.set(gameId, { process: null, startTime, polling: true })

      // Poll for the game process by name (if provided) or fallback to a timer
      const targetProcess = processName // e.g. "GameName.exe" — passed from renderer
      if (targetProcess) {
        // Wait up to 30s for the game process to appear, then poll until it exits
        let found = false
        const findAndWatch = async () => {
          for (let i = 0; i < 60; i++) { // wait up to 30s (60 × 500ms)
            await new Promise(r => setTimeout(r, 500))
            if (!runningGames.has(gameId)) return // manually stopped
            const pid = await getProcessPid(targetProcess)
            if (pid) { found = true; return pid }
          }
          return null
        }

        findAndWatch().then(async (pid) => {
          if (!pid) {
            // Could not find target process — fall back to a 5-minute assumed session
            console.warn(`[launch] Could not find process "${targetProcess}" after bat launch`)
            return
          }

          // Now poll until that PID disappears
          const pollExit = async () => {
            while (runningGames.has(gameId)) {
              await new Promise(r => setTimeout(r, 3000))
              const stillRunning = await getProcessPid(targetProcess)
              if (!stillRunning) {
                const entry = runningGames.get(gameId)
                if (!entry) return
                const mins = Math.max(0, Math.round((Date.now() - entry.startTime) / 60000))
      ...
      // If no processName given, tracking won't auto-stop — user must stop manually
      return { success: true }

    } else {
      // Original EXE path — unchanged, works perfectly
...
// Helper: find a running process by name and return its PID (Windows only)
function getProcessPid(exeName) {
  return new Promise((resolve) => {
    const { exec } = require('child_process')
    exec(`tasklist /FI "IMAGENAME eq ${exeName}" /FO CSV /NH`, (err, stdout) => {
      if (err || !stdout.includes(exeName)) return resolve(null)
...
ipcMain.handle('stop-game', async (_, gameId) => {
  ...
  try {
    runningGames.delete(gameId) // stop the poll loop first
    if (entry.process && entry.process.pid) {
      // Direct child process (non-Steam exe)
      ...
      // Steam/polling game — kill by exe name
      spawn('taskkill', ['/IM', entry.processName, '/f'], { stdio: 'ignore' })
    }
    const mins = Math.max(0, Math.round((Date.now() - entry.startTime) / 60000))
    if (win && !win.isDestroyed()) win.webContents.send('game-exited', { gameId, durationMins: mins, startTime: entry.startTime })
    return { success: true }
