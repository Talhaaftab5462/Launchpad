...
export function useGameLauncher({ onSessionEnd, onError }) {
  ...
  const launch = useCallback(async (game) => {
    ...
    if (!game.executablePath && !game.steamAppId) return { success: false, error: 'No executable set' }
    // processName = just the filename e.g. "NMS.exe" — used for PID polling
    const processName = game.executablePath ? game.executablePath.replace(/\\/g, '/').split('/').pop() : null
    const result = await window.launchpad.launchGame(game.id, game.executablePath, game.steamAppId, processName)
